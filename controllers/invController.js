...
invCont.addClassification = async function (req, res, next) {
  ...
  if (!classification_name || !/^[a-zA-Z0-9\s\-]+$/.test(classification_name)) {
    req.flash("notice", "Please provide a valid classification name (letters, numbers, spaces, and hyphens only).");
    res.render("inventory/add-classification", {
      title: "Add Classification",
      nav,
      errors: null,
...
invCont.addInventory = async function (req, res, next) {
  ...
  if (isNaN(parsedInvYear) || parsedInvYear < 1900 || parsedInvYear > new Date().getFullYear() + 1) {
    errors.push("Valid year is required.");
  }

  if (!inv_description) errors.push("Description is required.");

  if (!inv_image || !/^\/images\/vehicles\/.+\.(png|jpe?g|gif|webp)$/.test(inv_image)) {
    errors.push("Valid image path is required (e.g., /images/vehicles/image.png).");
  }

  if (!inv_thumbnail || !/^\/images\/vehicles\/.+-tn\.(png|jpe?g|gif|webp)$/.test(inv_thumbnail)) {
    errors.push("Valid thumbnail path is required (e.g., /images/vehicles/image-tn.png).");
  }

  const cleanedPrice = inv_price ? inv_price.replace(/,/g, '') : '';
  const parsedInvPrice = parseFloat(cleanedPrice);
  ...
  const cleanedMiles = inv_miles ? inv_miles.replace(/,/g, '') : '';
  const parsedInvMiles = parseFloat(cleanedMiles);
